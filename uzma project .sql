...
SELECT e.employee_id, e.last_name AS employee_lastname, e.manager_id, m.last_name AS manager_lastname
FROM employees e
...
SELECT d.department_name, COUNT(e.last_name) AS employee_count
FROM departments d
JOIN employees e ON d.department_id = e.department_id
GROUP BY d.department_name;
...
FROM employees e
JOIN departments d ON e.department_id = d.department_id
WHERE e.job_id REGEXP 'man|mgr'
  AND TIMESTAMPDIFF(YEAR, e.hire_date, CURDATE()) > 15;
  
  /*query 8*/
  
  SELECT CONCAT(e.first_name, ' ', e.last_name) AS employee_name, salary
FROM employees e
WHERE salary > (
  SELECT salary
  FROM employees
...
WHERE salary > (
  ...
  /*query 12*/
  /* If considered job grade is as department*/
